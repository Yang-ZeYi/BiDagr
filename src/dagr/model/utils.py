...
def postprocess_network_output(prediction, num_classes, conf_thre=0.01, nms_thre=0.65, height=640, width=640, filtering=True):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        # YOLOX-style score is obj_conf * class_conf (not squared).
        class_conf, class_pred = torch.max(image_pred[:, 5: 5 + num_classes], 1, keepdim=True)
        scores = image_pred[:, 4:5] * class_conf

        conf_mask = (scores.squeeze() >= conf_thre).squeeze()
        # Detections ordered as (x1, y1, x2, y2, score, class_pred)
        detections = torch.cat((image_pred[:, :4], scores, class_pred), 1)
