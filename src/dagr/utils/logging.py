...
class Checkpointer:
    def __init__(self, output_directory: Optional[Path] = None, args=None, optimizer=None, scheduler=None, ema=None, model=None, logger=None):
    ...
    def process(self, data: Dict[str, float], epoch: int):
        ...
        if self.logger is not None:
            self.logger.log(data, epoch=epoch)
...
class LocalMetricsLogger:
    def __init__(self, output_directory: Path, jsonl_name: str = "metrics.jsonl", csv_name: str = "metrics.csv"):
        self.output_directory = Path(output_directory)
        self.jsonl_path = self.output_directory / jsonl_name
        self.csv_path = self.output_directory / csv_name

        if not self.csv_path.exists() or self.csv_path.stat().st_size == 0:
            with self.csv_path.open("w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["time", "epoch", "step", "key", "value"])
    ...
    def log(self, data: Dict[str, Any], step: Optional[int] = None, epoch: Optional[int] = None):
        ...
        with self.csv_path.open("a", newline="") as f:
            writer = csv.writer(f)
            for key, value in data.items():
                writer.writerow([timestamp, epoch, step, key, self._normalize_value(value)])


def _generate_run_id(exp_name: str) -> str:
    stamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
    suffix = uuid4().hex[:6]
    if exp_name and exp_name != "temp":
        return f"{exp_name}-{stamp}-{suffix}"
    return f"run-{stamp}-{suffix}"


def set_up_logging_directory(dataset, task, output_directory, exp_name="temp", use_wandb: bool = True):
    ...
    if use_wandb and wandb is not None:
        wandb.init(project=project, id=exp_name, entity="yangzeyistudy", save_code=True, dir=str(output_directory))
        name = wandb.run.id
    else:
        if exp_name and exp_name != "temp":
            name = exp_name
        else:
            name = _generate_run_id(exp_name)
...
def log_hparams(args, output_directory: Optional[Path] = None, use_wandb: bool = True):
    hparams = {k: str(v) if type(v) is PosixPath else v for k, v in vars(args).items()}
    pprint(hparams)
    if output_directory is not None:
        with (Path(output_directory) / "hparams.json").open("w") as f:
...
def log_bboxes(data: Batch,
               ...
               key: str):
    if wandb is None:
        raise RuntimeError("wandb is not available for log_bboxes.")
